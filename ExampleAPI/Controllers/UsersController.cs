...
using static IdentityService.KPSPublicSoapClient;
    ...
    public async Task<IActionResult> Add([FromBody] User user)
    {
        using (KPSPublicSoapClient soapClient = new(EndpointConfiguration.KPSPublicSoap12))
        {
            var result = await soapClient.TCKimlikNoDogrulaAsync(
                TCKimlikNo: (long)Convert.ToDouble(user.IdentificationNumber),
                Ad: user.FirstName,
                Soyad: user.LastName,
                DogumYili: user.BirthYear
            );
            if (!result.Body.TCKimlikNoDogrulaResult)
                return BadRequest("There is no citizen matching to provided identity informations in the government system.");
            else
